...
// Tapping the chip opens a sheet with: search (placeholder), the box
// list (current marked with a brand check), and footer actions for
// "+ New box" and "Scan QR" (the latter still disabled until the
// pairing flow lands).
import SwiftUI
...
private struct BoxRow: View {
    ...
    private func tone(for state: BoxState) -> BadgeTone {
        switch state {
